...
void WSDisplayDriver(void)
{
    if (0 == _isDisplayInit)
    {
        ...
        esp_lcd_panel_reset(waveShareDisplay_panel); //must be called befor sp_lcd_panel_init()
...
#define bufferSize (H_COUNT*(V_COUNT/4)) //is divided by four to makesure the buffer isnt too big and breaks the display up nicely

uint16_t _test_color_data[bufferSize]; // Upload Buffer
 ...
 */
int t_initDisplayTest()
{

    for (int i = 0; i < bufferSize; i++) //upload buffer init
    {
        _test_color_data[i] = makeColor();
    }
